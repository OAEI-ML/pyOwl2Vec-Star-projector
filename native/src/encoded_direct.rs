        ...
        match self {
            ...
            Self::IgnoredDataPropertyAssertion {
                ...
                *counter = counter.checked_add(1).ok_or_else(|| {
                    KernelError::resource(if *negative {
...
impl<'a> DirectColumns<'a> {
    ...
    fn is_scope_mapped_ignored_class_assertion(
        ...
        let individual = self.field_node(start + 1)?;
        let candidate = match self.node_tag(class)? {
        ...
        for root_index in 0..self.root_count() {
            ...
            match self.node_tag(root)? {
                ...
                TAG_DATA_PROPERTY_ASSERTION | TAG_NEGATIVE_DATA_PROPERTY_ASSERTION => {
                    ...
                    let source = self.individual_value(self.field_node(start + 1)?, maximum_iri)?;
                    self.validate_literal(self.field_node(start + 2)?, maximum_iri)?;
                    if annotation_count != 0
                        || !matches!(
                            source,
...
fn own_composite_tail_projection(
    ...
    if matches!(
        ...
        if annotation_count != 0 {
            return Err(KernelError::unsupported(
                "bounded anonymous-scope data-property assertion requires an empty annotation set",
            ));
        }
        columns.named_data_property_iri(columns.field_node(field_start)?, max_iri_bytes)?;
        let source =
            columns.individual_value(columns.field_node(field_start + 1)?, max_iri_bytes)?;
        columns.validate_literal(columns.field_node(field_start + 2)?, max_iri_bytes)?;
        if !matches!(source, IndividualValue::Anonymous(_)) {
            return Err(KernelError::unsupported(
                "bounded anonymous-scope data-property assertion requires an anonymous source",
            ));
        }
    ...
    match projection {
        ...
        ClassAssertionProjection::Ignored
            if columns.is_scope_mapped_ignored_class_assertion(
                root,
                None,
                max_iri_bytes,
            )? =>
...
fn prepare_k_way_composite_batches_uncommitted<const N: usize>(
    ...
    for selected_count in selected_counts.iter().skip(1) {
        tail_root_count = tail_root_count
    ...
    {
        return Err(KernelError::malformed(
            "encoded bounded merge produced inconsistent root counts",
        ));
    }
    drop(merger);
    ...
    if projected > options.max_edges {
        return Err(KernelError::resource(format!(
            "encoded bounded composite requires {projected} edges; configured limit is {}",
            options.max_edges,
        )));
    }
...
mod tests {
    ...
    fn scope_mapped_nominal_class_assertion_fixture() -> Fixture {
        let mut fixture = Fixture::default();
        for iri in [b"urn:B".as_slice(), b"urn:Top", b"urn:i"] {
            fixture.push_scalar(COMPONENT_TEXT, iri);
            fixture.finish_node(TAG_IRI); // 1..=3
        }
        for (kind, iri_id) in [
        ...
        fixture.push_scalar(COMPONENT_BYTES, b"same");
        fixture.finish_node(TAG_ANONYMOUS_INDIVIDUAL); // 7
        fixture.push_node_set(&[7]);
        fixture.finish_node(TAG_OBJECT_ONE_OF); // 8
        fixture.push_node_ref(4);
        fixture.push_node_ref(5);
        fixture.push_empty_set();
    ...
    fn scope_mapped_data_property_assertion_fixture(assertion_tag: u16) -> Fixture {
        assert!(matches!(
            assertion_tag,
            TAG_DATA_PROPERTY_ASSERTION | TAG_NEGATIVE_DATA_PROPERTY_ASSERTION
        ));
        ...
        fixture.push_node_ref(10);
        fixture.push_empty_set();
        fixture.finish_node(assertion_tag); // 12
        fixture
            .root_kinds
            .extend_from_slice(&[ROOT_AXIOM, ROOT_AXIOM]);
        fixture.root_ids.extend_from_slice(&11_u32.to_le_bytes());
    ...
    #[test]
    fn two_member_composite_remaps_skipped_data_assertion_scopes() {
        for (assertion_tag, negative) in [
            (TAG_DATA_PROPERTY_ASSERTION, false),
            (TAG_NEGATIVE_DATA_PROPERTY_ASSERTION, true),
        ] {
            let left = scope_mapped_data_property_assertion_fixture(assertion_tag);
            let right = scope_mapped_data_property_assertion_fixture(assertion_tag);
            let mut left_scope_map = [0_u8; 64];
            left_scope_map[..32].fill(7);
            left_scope_map[32..].fill(8);
            let mut right_scope_map = [0_u8; 64];
            ...
            .unwrap();
            assert_eq!(
                asserted.statistics().data_property_assertions,
                usize::from(!negative) * 2
            );
            assert_eq!(
    ...
    #[test]
    fn four_table_composite_deduplicates_all_selected_source_roots() {
        let first = named_subclass_delta_fixture(b"urn:A", b"urn:B");
        let second = named_subclass_delta_fixture(b"urn:A", b"urn:B");
        let third = named_subclass_delta_fixture(b"urn:A", b"urn:B");
        let fourth = named_subclass_delta_fixture(b"urn:A", b"urn:B");
        let options = DirectCompileOptions {
            bidirectional: false,
            asserted_taxonomy_only: false,
            only_taxonomy: false,
        ...
        assert_eq!(
            ...
            first.columns().buffer_bytes().unwrap()
                + second.columns().buffer_bytes().unwrap()
                + third.columns().buffer_bytes().unwrap()
                + fourth.columns().buffer_bytes().unwrap()
        );
        assert!(prepared.preparation.overlay_deltas.is_empty());
    ...
    #[test]
    fn canonical_four_root_cursor_merges_in_one_selected_k_way_pass() {
        let first = canonical_text_roots(&[b"a", b"e"]);
        let second = canonical_text_roots(&[b"b", b"e"]);
        ...
        assert_eq!(merger.next(&running_state()).unwrap(), None);
        let report = merger.report();
        assert_eq!(report.roots_emitted, 4);
        assert_eq!(report.deduplicated_roots, 3);
        assert!(report.work > 0);
        assert!(report.workspace_bytes > 0);
        assert!(report.canonical_bytes_compared > 0);
    }
