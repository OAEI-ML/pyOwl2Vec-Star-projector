...
| Direct/mmap/overlay/composite support | Exact full bytes and the canonical eleven-column packed direct-bytes arena are supported. Bounded empty-overlay aliases, one-root local deltas, dynamic exact-direct composites with source-local selectors, and reviewed nested-overlay composite families execute without flattening; the detailed sections above define their narrower annotation and scope-map envelopes. Kernel v120 adds two independent source-local `EXCLUDE` layers across one nested overlay base and its distinct direct sibling. Kernel v119 isolates valid general exporters at one future `PyUntypedBuffer` retention seam and proves mmap cleanup plus hostile-layout rejection without copying, but the pinned `abi3-py310` floor cannot use the Stable-ABI buffer lease introduced in 3.11. General recursive segmented traversal, selection on a nested overlay member, nested aliases/deltas beyond the bounded families, mmap execution, annotation-sensitive aliases outside the reviewed plans, and public dispatch remain unsupported by the Rust path |
