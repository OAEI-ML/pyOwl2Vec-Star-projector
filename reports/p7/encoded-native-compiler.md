...
| Direct/mmap/overlay/composite support | Exact full bytes and the canonical eleven-column packed direct-bytes arena are supported. A bounded chain of canonical empty `OVERLAY_BASE`/`ALL` aliases to an exact-direct source is installed-wheel proven without flattening, with every owner retained and public depth/work bounds enforced; edited, posted, annotation-sensitive, mmap, composite, and other segmented families remain unsupported by the Rust path |
...
3. expand the proven no-copy recursive empty-overlay owner retention to posted overlays, local
   deltas, mmap, composites, and general segment traversal;
