...
### Bounded one-root HasKey delta

Revision `1a7053cdf6e5fb00c52e6894738edc32e6961298` admits one unannotated local
`HasKey` through kernel v66. The top manifest remains exactly `OVERLAY_BASE` plus
`OVERLAY_DELTA`; its base may use `ALL` or the existing one exact nonempty `EXCLUDE` posting
table, while the delta remains `ALL`. The key axiom may use the complete already-bounded recursive
class-expression envelope, a canonical set of named or inverse object-property expressions, and a
canonical set of named data properties. At least one property is required across the two sets.
An annotated axiom, duplicate base/local identity, second local root, empty or noncanonical
property envelope, or other unsupported local shape retains whole-operation fallback or typed
pre-output rejection.

Rust requires the root tag and exact has-key constructor count to agree, validates the class
expression and both property sets recursively, and rechecks strict canonical set order and
uniqueness before using the bounded canonical merger. The axiom emits no edge and retains no owned
emitting delta record. Normal and `only_taxonomy` projection preserve the exact has-key count and
skipped-axiom diagnostic; asserted-taxonomy preserves the constructor count but suppresses that
diagnostic, matching the pinned scalar behavior. The base edge total is checked against the caller
bound before publication. No selected-root index, flattened base, copied structural graph,
canonical-byte arena, scalar ontology value, complete output vector, or per-row FFI call is
created.

The exact installed matrix crosses independent- and packed-bytes exporters, object-only,
data-only, mixed named, and recursively nested/inverse-property key shapes, and an `ALL` base, one
partial base exclusion, exclusion of every base root, and `only_taxonomy`: 32 supported executions
with exact scalar ordered-edge/report parity. Twenty-nine adjacent integration cases retain
whole-call fallback, now including an annotated local `HasKey`. The representative object-only
`ALL` case reports 768 structural bytes, 22 retained zero-copy buffers, one skipped has-key axiom,
and two one-edge base batches; the recursive/inverse mixed case reports 963 bytes with identical
projected edges. A partial exclusion retains the exact four-byte posting exporter as the
twenty-third native input and emits one base edge. Excluding both base roots retains an eight-byte
posting table, publishes the local root and diagnostic with zero edges, and performs only the
initial native boundary call. Both provider layouts have identical results, statistics, and
...
The exact `1a7053c` archive built a fresh release-profile abi3 wheel and passed all 1,543 tests
from the isolated installed projector/core payload. All 411 private integration cases, the 61
focused HasKey/adjacent-fallback cases, 15 benchmark cases, 73 Rust tests, rustfmt, Clippy with
warnings denied, Ruff, strict mypy over 17 source files, the installed smoke, runtime dependency
boundary, and 30-member native-wheel audit passed. The public feature ledger remains exactly
`abi3-py310` and `bounded-batches`; ordinary public native projection still uses scalar-native
with zero native counters. Exact archive, wheel, installed-payload, counter, and gate hashes are
recorded in
[`installed-one-root-has-key-delta-checkpoint.json`](evidence/installed-one-root-has-key-delta-checkpoint.json).
This remains focused private correctness evidence, not another local axiom family,
annotated/multi-root delta, local deletion, `INCLUDE`, nested-delta, composite, mmap, corpus,
performance, public-dispatch, or release-acceptance evidence.

### Bounded one-root object-property axioms delta

Revision `dadb44dd5820295ff09a7ecebc3a9b403f1e47c3` admits one unannotated local
...
`SymmetricObjectProperty`, `AsymmetricObjectProperty`, or `TransitiveObjectProperty` through
kernel v67. The top manifest remains exactly `OVERLAY_BASE` plus `OVERLAY_DELTA`; its base may use
`ALL` or the existing one exact nonempty `EXCLUDE` posting table, while the delta remains `ALL`.
The two set constructors accept a canonical binary or ternary set of named or inverse named
object-property expressions. Each unary characteristic accepts one named or inverse named
object-property expression. An annotated axiom, oversized or noncanonical set, duplicate
base/local identity, second local root, or other unsupported local shape retains whole-operation
fallback or typed pre-output rejection.

Rust uses one root-tag/count classifier for the nine shapes, validates the complete local table,
rechecks each property expression and the strict canonical order and uniqueness of either set, and
then uses the bounded canonical merger. Every admitted axiom is silent: it emits no edge and
retains no owned emitting delta record. Normal and `only_taxonomy` projection preserve the exact
constructor count and skipped-axiom diagnostic; asserted-taxonomy preserves the constructor count
but suppresses that diagnostic, matching the pinned scalar behavior. The base edge total is
checked against the caller bound before publication. No selected-root index, flattened base,
copied structural graph, canonical-byte arena, scalar ontology value, complete output vector, or
per-row FFI call is created.

The exact installed matrix crosses independent- and packed-bytes exporters, all nine constructor
shapes, and an `ALL` base, one partial base exclusion, exclusion of every base root, and
`only_taxonomy`: 72 supported executions with exact scalar ordered-edge/report parity. Forty
adjacent integration cases retain whole-call fallback, including every annotated constructor and
oversized property sets. The representative ternary equivalent-property `ALL` case reports 910
structural bytes, 22 retained zero-copy buffers, one skipped equivalent-property axiom, and two
one-edge base batches. A binary disjoint-property partial exclusion reports 782 bytes and retains
the exact four-byte posting exporter as the twenty-third native input. An inverse-functional
property with both base roots excluded reports 637 bytes, retains the eight-byte posting table,
publishes the local root and diagnostic with zero edges, and performs only the initial native
boundary call. A taxonomy-only transitive-property case reports 610 bytes and the same two base
edges. Both provider layouts have identical results, statistics, and counters.

The exact `dadb44d` archive built a fresh release-profile abi3 wheel and passed all 1,626 tests
from the isolated installed projector/core payload. All 494 private integration cases, the 112
focused object-property/adjacent-fallback cases, 15 benchmark cases, 75 Rust tests, rustfmt,
Clippy with warnings denied, Ruff, strict mypy over 17 source files, the installed smoke, runtime
dependency boundary, and 30-member native-wheel audit passed. The public feature ledger remains
exactly `abi3-py310` and `bounded-batches`; ordinary public native projection still uses
scalar-native with zero native counters. Exact archive, wheel, installed-payload, counter, and
gate hashes are recorded in
[`installed-one-root-object-property-axioms-delta-checkpoint.json`](evidence/installed-one-root-object-property-axioms-delta-checkpoint.json).
This remains focused private correctness evidence, not another local axiom family,
annotated/multi-root delta, local deletion, `INCLUDE`, nested-delta, composite, mmap, corpus,
performance, public-dispatch, or release-acceptance evidence.

### Bounded one-root annotation-property axioms delta

Revision `66d8f0514f52720da4673566dd9f7deef80ef573` admits one unannotated local
`SubAnnotationPropertyOf`, `AnnotationPropertyDomain`, or `AnnotationPropertyRange` through kernel
v68. The top manifest remains exactly `OVERLAY_BASE` plus `OVERLAY_DELTA`; its base may use `ALL`
or the existing one exact nonempty `EXCLUDE` posting table, while the delta remains `ALL`.
Sub-property axioms require two named annotation properties. Domain and range axioms require one
named annotation property and one IRI target. An annotated axiom, duplicate base/local identity,
second local root, or other unsupported local shape retains whole-operation fallback or typed
pre-output rejection.

Rust uses one root-tag/count classifier for the three shapes and reuses the complete strict direct
validators for their property entities, target IRIs, node arity, and annotation graph. The local
envelope additionally requires an empty axiom annotation set before invoking the bounded canonical
merger. Every admitted axiom is silent: it emits no edge and retains no owned emitting delta
record. Normal and `only_taxonomy` projection preserve the exact constructor count and
skipped-axiom diagnostic; asserted-taxonomy preserves the constructor count but suppresses that
diagnostic, matching the pinned scalar behavior. The base edge total is checked against the caller
bound before publication. No selected-root index, flattened base, copied structural graph,
canonical-byte arena, scalar ontology value, complete output vector, or per-row FFI call is
...
The exact installed matrix crosses independent- and packed-bytes exporters, all three constructor
shapes, and an `ALL` base, one partial base exclusion, exclusion of every base root, and
`only_taxonomy`: 24 supported executions with exact scalar ordered-edge/report parity.
Forty-three adjacent integration cases retain whole-call fallback, including all three annotated
annotation-property constructors. The representative sub-property `ALL` case reports 746
structural bytes, 22 retained zero-copy buffers, one skipped sub-annotation-property axiom, and
two one-edge base batches. A domain partial exclusion reports 668 bytes and retains the exact
four-byte posting exporter as the twenty-third native input. A range axiom with both base roots
excluded reports 667 bytes, retains the eight-byte posting table, publishes the local root and
diagnostic with zero edges, and performs only the initial native boundary call. The taxonomy-only
range case has the same 667 structural bytes and two base edges. Both provider layouts have
identical results, statistics, and counters.

The exact `66d8f05` archive built a fresh release-profile abi3 wheel and passed all 1,653 tests
from the isolated installed projector/core payload. All 521 private integration cases, the 67
focused annotation-property/adjacent-fallback cases, 15 benchmark cases, 76 Rust tests, rustfmt,
Clippy with warnings denied, Ruff, strict mypy over 17 source files, the installed smoke, runtime
dependency boundary, and 30-member native-wheel audit passed. The public feature ledger remains
exactly `abi3-py310` and `bounded-batches`; ordinary public native projection still uses
scalar-native with zero native counters. Exact archive, wheel, installed-payload, counter, and
gate hashes are recorded in
[`installed-one-root-annotation-property-axioms-delta-checkpoint.json`](evidence/installed-one-root-annotation-property-axioms-delta-checkpoint.json).
This remains focused private correctness evidence, not another local axiom family,
annotated/multi-root delta, local deletion, `INCLUDE`, nested-delta, composite, mmap, corpus,
performance, public-dispatch, or release-acceptance evidence.
...
### Bounded one-root ignored-class delta

Revision `5c2e817fd24aa378d37fb4f3774b8c8e365602ab` admits one unannotated local
`SubClassOf` or `ClassAssertion` root through kernel v70 only when the complete direct projection
classifier marks that root ignored. The top manifest remains exactly `OVERLAY_BASE` plus
`OVERLAY_DELTA`; its base may use `ALL` or the existing one exact nonempty `EXCLUDE` posting table,
while the delta remains `ALL`. The complete recursive class-expression graph is revalidated and
must reach no anonymous individual until a local anonymous-scope remap is defined. An annotated
root, an anonymous-scope-bearing class assertion, duplicate base/local identity, second local
root, or other unsupported local shape retains whole-operation fallback or typed pre-output
rejection.

Rust uses one exact root-tag/count classifier for the two constructors and then re-runs the
existing complete `SubClassOf` or `ClassAssertion` projection classifier. Admission requires that
second result to remain ignored after all direct structural and IRI validation, and requires an
empty axiom annotation set. Every admitted axiom is silent: it emits no edge and retains no owned
emitting delta record. Normal, `only_taxonomy`, and asserted-taxonomy projection preserve the
exact constructor and ignored-shape counters while leaving `skipped_axioms` at zero. The Python
report derives the matching `MOWL_IGNORED_SHAPE` diagnostic. The base edge total is checked
against the caller bound before publication. No selected root index, flattened base, copied
structural graph, canonical-byte arena, scalar ontology value, complete output vector, or per-row
FFI call is created.

The exact installed matrix crosses independent- and packed-bytes exporters, four representative
ignored shapes, and an `ALL` base, one partial base exclusion, exclusion of every base root, and
`only_taxonomy`: 32 supported executions with exact scalar ordered-edge/report parity.
Forty-seven adjacent integration cases retain whole-call fallback, including annotated
projecting and ignored class shapes plus anonymous class assertions without a local scope remap.
The representative complex-restriction `SubClassOf` `ALL` case reports 1,037 structural bytes,
22 retained zero-copy buffers, one ignored-subclass diagnostic, zero skipped axioms, and two
one-edge base batches. An aggregate `SubClassOf` partial exclusion reports 883 bytes and retains
the exact four-byte posting exporter as the twenty-third native input. A complex-restriction
`ClassAssertion` with both base roots excluded reports 1,042 bytes, retains the eight-byte posting
table, publishes the ignored root and diagnostic with zero edges, and performs only the initial
native boundary call. A taxonomy-only recursive `ClassAssertion` reports 915 bytes and the same
two base edges. Both provider layouts have identical results, statistics, and counters.

The exact `5c2e817` archive built a fresh release-profile abi3 wheel and passed all 1,723 tests
from the isolated installed projector/core payload. All 591 private integration cases, the 81
focused ignored-class/adjacent-fallback cases, 15 benchmark cases, 78 Rust tests, rustfmt, Clippy
with warnings denied, Ruff, strict mypy over 17 source files, the installed smoke, runtime
dependency boundary, and 30-member native-wheel audit passed. The public feature ledger remains
exactly `abi3-py310` and `bounded-batches`; ordinary public native projection still uses
scalar-native with zero native counters. Exact archive, wheel, installed-payload, counter, and
gate hashes are recorded in
[`installed-one-root-ignored-class-delta-checkpoint.json`](evidence/installed-one-root-ignored-class-delta-checkpoint.json).
This remains focused private correctness evidence, not another local axiom family, anonymous-scope
remapping, annotated/multi-root delta, local deletion, `INCLUDE`, nested-delta, composite, mmap,
corpus, performance, public-dispatch, or release-acceptance evidence.

### Bounded one-root ignored-equivalence delta

Revision `3bd4104419cb9c8f6ab1db01d014f203ff4e1f74` admits one unannotated local
`EquivalentClasses` root through kernel v71 only when the complete direct equivalence projection
classifier marks that whole root ignored. The top manifest remains exactly `OVERLAY_BASE` plus
`OVERLAY_DELTA`; its base may use `ALL` or the existing one exact nonempty `EXCLUDE` posting table,
while the delta remains `ALL`. The candidate must be a canonical binary or ternary recursive
class-expression set. Its complete graph is revalidated and must reach no anonymous individual
until a local anonymous-scope remap is defined. A projecting pair or aggregate equivalence,
annotated or oversized root, duplicate base/local identity, second local root, or other unsupported
local shape retains whole-operation fallback or typed pre-output rejection.

Rust uses one exact root-tag/count classifier and then re-runs the existing complete equivalence
projection classifier. Admission requires that second result to remain ignored after all recursive
structural and IRI validation, requires an empty axiom annotation set, and rejects projecting
variants because they can emit pair edges or contribute role-expanded products. Every admitted
axiom is silent: it emits no edge and retains no owned emitting delta record. Normal and
`only_taxonomy` projection preserve `equivalents=1` and `ignored_equivalents=1`; asserted-taxonomy
preserves the constructor count while suppressing the ignored-shape count, matching the complete
direct path. `aggregate_equivalents` and `skipped_axioms` remain zero in every mode. The Python
report derives the matching `MOWL_IGNORED_SHAPE` diagnostic outside asserted-taxonomy. The base edge
total is checked against the caller bound before publication. No selected root index, flattened
base, copied structural graph, canonical-byte arena, scalar ontology value, complete output vector,
or per-row FFI call is created.

The exact installed matrix crosses independent- and packed-bytes exporters, four representative
ignored equivalence shapes, and an `ALL` base, one partial base exclusion, exclusion of every base
root, and `only_taxonomy`: 32 supported executions with exact scalar ordered-edge/report parity.
Fifty-two adjacent integration cases retain whole-call fallback, including projecting pair and
aggregate equivalences, annotated and anonymous-scope-bearing ignored equivalences, and oversized
ignored equivalences. The representative named/restriction `ALL` case reports 1,054 structural
bytes, 22 retained zero-copy buffers, one ignored-equivalence diagnostic, zero skipped axioms, and
two one-edge base batches. An all-complex partial exclusion reports 914 bytes and retains the exact
four-byte posting exporter as the twenty-third native input. A recursive ignored equivalence with
both base roots excluded reports 927 bytes, retains the eight-byte posting table, publishes the
local root and diagnostic with zero edges, and performs only the initial native boundary call. A
taxonomy-only ternary ignored equivalence reports 1,037 bytes and the same two base edges. Both
provider layouts have identical results, statistics, and counters.

The exact `3bd4104` archive built a fresh release-profile abi3 wheel and passed all 1,761 tests
from the isolated installed projector/core payload. All 629 private integration cases, the 85
focused ignored-equivalence/adjacent-fallback cases, 15 benchmark cases, 79 Rust tests, rustfmt,
Clippy with warnings denied, Ruff, strict mypy over 17 source files, the installed smoke, runtime
dependency boundary, and 30-member native-wheel audit passed. The public feature ledger remains
exactly `abi3-py310` and `bounded-batches`; ordinary public native projection still uses
scalar-native with zero native counters. Exact archive, wheel, installed-payload, counter, and gate
hashes are recorded in
[`installed-one-root-ignored-equivalence-delta-checkpoint.json`](evidence/installed-one-root-ignored-equivalence-delta-checkpoint.json).
This remains focused private correctness evidence, not projecting local equivalence support,
anonymous-scope remapping, annotated/multi-root delta, local deletion, `INCLUDE`, nested-delta,
composite, mmap, corpus, performance, public-dispatch, or release-acceptance evidence.

### General-buffer retention boundary at the Python 3.10 ABI floor

Kernel v119 isolates buffer metadata validation from exporter-specific lifetime ownership.
Readonly, one-dimensional, unsigned-byte, C-contiguous memoryviews now become explicit validated
candidates. Exact immutable `bytes` candidates continue through the existing full-exporter or
canonical packed-arena zero-copy paths. A valid mmap, readonly bytearray, or other general
candidate reaches one dedicated retention boundary and raises the private typed unsupported error;
it is never converted to `bytes`, staged into Rust ownership, or admitted to output. The retained
storage enum is the single future implementation seam for a leased general-buffer variant.

This fail-closed result is required by the pinned ABI, not by an untested assumption. PyO3 0.28.3
gates its complete `buffer` module with
`cfg(any(not(Py_LIMITED_API), Py_3_11))`. Its FFI crate likewise exports `Py_buffer`,
`PyObject_GetBuffer`, and `PyBuffer_Release` only under `Py_3_11`. The local extension build
demonstrably emits `Py_LIMITED_API` and version cfgs only through `Py_3_10`. CPython documents the
buffer structure, request flags, acquisition, and release functions as Stable ABI only since
Python 3.11. The deprecated old pointer-only buffer functions cannot retain a releasable lease and
are not used. Raising the wheel floor, using an unsafe private struct layout, retaining an
unreleasable pointer, or copying mmap columns were all kept outside this checkpoint.

Eleven focused source-tree cases pass after rebuilding the abi3 extension: exact full and packed
bytes acceptance, packed-layout rejection, owner lifetime, descriptor failure atomicity, explicit
readonly mmap rejection and cleanup, readonly bytearray rejection, and hostile writable, strided,
multidimensional, and signed-format rejection. Cargo check, Clippy with warnings denied, and Ruff
also pass, as do all 136 Rust unit tests in the non-extension link mode. These are local fast checks
only. The feature ledger remains exactly `abi3-py310` and
`bounded-batches`; `encoded-structural-compiler-v1` remains absent. General/mmap zero-copy support
therefore remains open until a reviewed Python 3.11-or-newer ABI policy can supply the real retained
lease and its full lifecycle/platform evidence.

### Independent exclusions across one nested overlay composite

Kernel v120 admits one bounded canonical three-table traversal: an outer two-member composite
contains one already-supported one-local-root overlay and one distinct exact-direct sibling. The
nested overlay's direct base may use `ALL` or its existing source-local `EXCLUDE`; independently,
the outer direct-sibling member may now use one exact nonempty `EXCLUDE`. Rust validates the outer
manifest's exact source, owner, member token, posting mode, and posting-exporter identity before
output, retains both posting exporters, and applies each only to its source table during the same
canonical merge. It creates no selected-root index, flattened base, structural staging copy,
scalar ontology value, or per-row FFI call.

Outer selection on the nested overlay member, `INCLUDE`, anonymous-scope remapping, another nested
delta, and the corresponding broader recursive traversal families remain fail-closed. The current
pyowl-core composition helper materializes this specific nested-removal request, so the focused
test publishes an otherwise canonical manifest derived from real independent- and packed-bytes
sources while projecting an equivalent semantic view. Two exporter-layout parity cases, exact
posting-identity and out-of-range rejection, unsupported-selector rejection, and retained-member
lifetime pass locally. The two-selection case retains 35 native inputs, reports eight posting
bytes, and preserves exact scalar order, statistics, diagnostics, and four-edge output. The
feature ledger remains exactly `abi3-py310` and `bounded-batches`;
`encoded-structural-compiler-v1` remains absent.

### Independent exclusions across two direct siblings

Kernel v121 extends the preceding bounded family to one outer three-member composite containing
the same one-local-root nested overlay and two distinct exact-direct siblings. Either direct
sibling, or both together, may carry one exact nonempty source-local `EXCLUDE`; the nested
overlay's direct base independently retains its existing `ALL`/`EXCLUDE` choice. The Python
resolver preserves each posting exporter in manifest order, and Rust revalidates each source,
owner, member token, posting mode, and exact posting-exporter identity before output. Each selector
is binary-searched only against its own source table during the same canonical four-table merge.

First-only, second-only, and dual direct selection pass against both independent- and packed-bytes
providers with exact scalar order, statistics, diagnostics, and five- or six-edge output. The
single-direct-selector cases retain 46 native inputs and report eight posting bytes; the
dual-direct-selector cases retain 47 native inputs and report twelve posting bytes. All cases
...
`abi3-py310` and `bounded-batches`; `encoded-structural-compiler-v1` remains absent.

### Source-local selection on the nested member

Kernel v122 admits one exact outer `EXCLUDE` on the nested overlay member in the bounded
one-direct-sibling family. A V1 posting on that nested source can address only roots in the
overlay's local delta columns; inherited base roots are deliberately not assigned recursive
locators. The adapter therefore retains that exact posting exporter as the local-delta selector,
while separately retaining the overlay's inner base selector and the direct sibling's outer
selector. Rust revalidates the outer manifest's source, owner, token, mode, and exact exporter
identity before binary-searching each of the three selectors against only its own table.

The triple-selector case passes against independent- and packed-bytes providers with exact scalar
order, statistics, diagnostics, and three-edge output. It retains 36 native inputs, reports twelve
posting bytes, and reports zero selected-root indexes, base flattening, structural staging copies,
scalar ontology materializations, and per-row FFI calls. Replacement nested-selector exporters,
out-of-range nested postings, and outer `INCLUDE` fail before output. The retained manifest,
nested overlay, base, and sibling views survive exactly until compiler release. The packed-provider
case also passes byte-identical sink batches, canonical digest, and portable artifact output
through the hidden consumer surfaces without scalar fallback.

Scope remapping, another nested delta, selection on a nested member with two direct siblings, and
the corresponding broader recursive traversal families remain fail-closed. The feature ledger
remains exactly `abi3-py310` and `bounded-batches`; `encoded-structural-compiler-v1` remains absent.

### Nested-member selection with two direct siblings

Kernel v123 carries the v122 source-local nested-member rule into the exact four-table family. The
outer nested-member `EXCLUDE`, either or both direct-sibling `EXCLUDE` tables, and the overlay's
inner base `EXCLUDE` remain four independent exporters. The Python resolver preserves each
selector in manifest identity order, and Rust revalidates the nested source, both direct sources,
owners, tokens, modes, and exact exporter identities before the canonical four-table merge.

The all-four-selector case passes against independent- and packed-bytes providers with exact scalar
order, statistics, diagnostics, and four-edge output. It retains 48 native inputs, reports sixteen
posting bytes, and reports zero selected-root indexes, base flattening, structural staging copies,
scalar ontology materializations, and per-row FFI calls. Replacement nested/direct exporters,
out-of-range nested/direct postings, and outer `INCLUDE` fail before output. All five retained
views survive exactly until compiler release, and the packed-provider plan passes byte-identical
sink batches, canonical digest, and portable artifact output without scalar fallback.

Scope remapping, another nested delta, a third direct sibling, and the corresponding broader
recursive traversal families remain fail-closed. The feature ledger remains exactly `abi3-py310`
and `bounded-batches`; `encoded-structural-compiler-v1` remains absent.

### Scope remapping across a fully excluded local delta

Kernel v124 composes the reviewed anonymous-scope remapping path with the exact
one-direct-sibling nested-overlay family when its one named-to-named `SubClassOf` local-delta root
is fully selected out. The outer nested-member `EXCLUDE` remains attached to the local-delta
table; the Python resolver requires the complete sorted sequence of local positions and preserves
the exact exporter identity. Rust revalidates that selector and the excluded root's complete
named-subclass shape before the canonical three-table merge. The inner overlay base and both
scope-mapped member tables remain `ALL`, so the selector cannot affect either mapped construct or
an inherited root.

Both independent- and packed-bytes provider cases pass with exact scalar order, statistics,
diagnostics, and two distinct remapped object-assertion edges. The selected plan retains 36 native
inputs and reports 132 posting bytes: two 64-byte anonymous-scope maps plus one four-byte
local-delta selector. It reports zero selected-root indexes, base flattening, structural staging
copies, scalar ontology materializations, and per-row FFI calls. Replacement selector or
scope-map exporters, out-of-range or partial selectors, `INCLUDE`, and selection on the direct
mapped member fail before output. The manifest, nested overlay, base, and direct member views
survive exactly until compiler release, and the selected plan passes byte-identical sink batches,
canonical digest, and portable artifact output without scalar fallback.

Partial local-delta selection, selection on either mapped member or the inner base, scope remapping
with two direct siblings, another nested delta, and broader recursive traversal remain
fail-closed. The feature ledger remains exactly `abi3-py310` and `bounded-batches`;
`encoded-structural-compiler-v1` remains absent.

### Scope remapping with a second neutral direct sibling

Kernel v125 extends the v124 scope-mapped nested family to four retained merge tables by admitting
one additional unannotated named-subclass-only direct sibling. The nested local-delta table and the
new neutral sibling independently accept `ALL` or a complete source-local `EXCLUDE`; the inner
overlay base and both anonymous-scope-mapped construct tables remain `ALL`. This uses the existing
four table inputs and two scope-map inputs: no private ABI or feature-ledger change is required.
Rust now admits exactly two mapped members and at most two neutral tables, and the manifest
validator binds the two maps and either selector to their exact source identities regardless of
canonical outer-member order.

All four selector combinations pass against independent- and packed-bytes providers with exact
scalar order, statistics, diagnostics, and two through four output edges. The plans retain 46
through 48 native inputs and report 128 through 136 posting bytes. Each reports 55 zero-copy
structural buffers, four referenced views, eight segments, and zero selected-root indexes, base
flattening, structural staging copies, scalar ontology materializations, or per-row FFI calls.
Replacement selector or scope-map exporters, out-of-range postings, and selection on the mapped
direct member fail before output. All five retained views survive exactly until compiler release,
and the dual-selector plan passes byte-identical sink batches, canonical digest, and portable
artifact output without scalar fallback.

A third mapped member requires another scope-map input and remains outside the fixed ABI.
Selection on either mapped member or the inner base, partial neutral selection, `INCLUDE`, another
nested delta, and broader recursive traversal remain fail-closed. The feature ledger remains
exactly `abi3-py310` and `bounded-batches`; `encoded-structural-compiler-v1` remains absent.

### Source-local selection inside a scope-mapped nested base

Kernel v126 permits the nested overlay's mapped base table to carry an exact source-local
`EXCLUDE` when every selected position is an unannotated named-to-named `SubClassOf` root. The
anonymous-scope-bearing construct remains selected and is still required to bind the exact
32-byte source scope in both maps. Python validates the selector against the retained base table
before choosing the private plan; Rust repeats complete validation for excluded roots before the
same three- or four-table canonical merge. The base selector remains independent of the outer
local-delta and neutral-sibling selectors introduced by v124-v125.

Both independent- and packed-bytes provider cases pass with exact scalar order, statistics,
diagnostics, and two distinct remapped object-assertion edges. The selected three-table plan
retains 37 native inputs and reports 136 posting bytes. Across all four outer-selector
combinations, the four-table plans retain 47 through 49 native inputs and report 132 through 140
posting bytes. They continue to report 44 or 55 zero-copy structural buffers, three or four
referenced views, six or eight segments, and zero selected-root indexes, base flattening,
structural staging copies, scalar ontology materializations, or per-row FFI calls. Replacement
base-selector exporters fail exact manifest binding; an excluded mapped construct fails before
output. The retained views survive exactly until compiler release, and the all-selector
four-table plan passes byte-identical sink batches, canonical digest, and portable artifact output
without scalar fallback.

`INCLUDE`, annotated or non-taxonomy base exclusions, selection of the mapped construct, a third
mapped member, another nested delta, and general recursive traversal remain fail-closed. The
feature ledger remains exactly `abi3-py310` and `bounded-batches`;
`encoded-structural-compiler-v1` remains absent.

### Independent selection inside both scope-mapped members

Kernel v127 applies the v126 source-local rule independently to the second scope-mapped direct
member through the existing third-table selector input. Its exact `EXCLUDE` may contain only
unannotated named-to-named `SubClassOf` roots; the anonymous-scope-bearing construct remains
selected. Python binds the selector to that retained member and verifies its narrow root shape
before plan selection. Rust repeats the complete root and annotation validation before the same
single parameterized `ROOT_RULES` traversal. No private ABI or feature-ledger expansion is
required, and the mapped-base, mapped-direct, local-delta, and neutral-sibling selectors remain
independent.

Both independent- and packed-bytes provider cases pass every selector combination with exact
scalar order, statistics, diagnostics, and output parity. The three-table family retains 36
through 38 native inputs and reports 132 through 140 posting bytes; the all-three-selector case
retains 38 inputs and reports 140 bytes. The four-table family retains 46 through 50 inputs and
reports 128 through 144 posting bytes across all 32 provider-independent selector combinations.
It continues to report 44 or 55 zero-copy structural buffers, three or four referenced views, six
or eight segments, and zero selected-root indexes, base flattening, structural staging copies,
scalar ontology materializations, or per-row FFI calls. Replacement or cloned selectors fail
exact manifest binding; out-of-range postings and exclusion of either mapped construct fail before
output. Retained-view lifetime, sink, digest, and portable-artifact consumers preserve byte-exact
parity.

`INCLUDE`, annotated or non-taxonomy mapped-member exclusions, selection of either mapped
construct, a third mapped member, another nested delta, and general recursive traversal remain
fail-closed. The feature ledger remains exactly `abi3-py310` and `bounded-batches`;
`encoded-structural-compiler-v1` remains absent.
...
| Direct/mmap/overlay/composite support | Exact full bytes and the canonical eleven-column packed direct-bytes arena are supported. Bounded empty-overlay aliases, one-root local deltas, dynamic exact-direct composites with source-local selectors, and reviewed nested-overlay composite families execute without flattening; the detailed sections above define their narrower annotation and scope-map envelopes. Kernel v121 applies independent source-local `EXCLUDE` layers to one nested overlay base and either or both of its two distinct direct siblings. Kernels v122-v123 additionally apply an outer nested-member `EXCLUDE` to that overlay's exact local delta table with one or two direct siblings. Kernel v124 composes the one-direct scope-mapped nested family with a complete `EXCLUDE` of its one named-subclass local-delta root while keeping both mapped members and the inner base at `ALL`. Kernel v125 adds a second named-subclass-only neutral direct sibling and independently permits complete exclusions on that sibling and the local delta while retaining exactly two `ALL` mapped members. Kernel v126 permits the mapped nested base to exclude only exact unannotated named-taxonomy roots while retaining its anonymous construct, independently composing all three selector layers. Kernel v127 independently permits the same narrow exclusion on the second scope-mapped direct member while retaining its mapped construct and composing all four selector layers through the fixed ABI. Kernel v119 isolates valid general exporters at one future `PyUntypedBuffer` retention seam and proves mmap cleanup plus hostile-layout rejection without copying, but the pinned `abi3-py310` floor cannot use the Stable-ABI buffer lease introduced in 3.11. General recursive segmented traversal beyond these bounded families, nested aliases/deltas, another mapped sibling requiring a third scope-map input, mmap execution, annotation-sensitive aliases outside the reviewed plans, and public dispatch remain unsupported by the Rust path |
...
| Kernel v70 bounded local-delta addendum | One unannotated ignored-shape `SubClassOf` or `ClassAssertion` is now admitted through the silent transaction over an `ALL` or one exact base `EXCLUDE` table. The complete direct projection classifier must return ignored, the graph must reach no anonymous individual, the constructor and ignored-shape counters remain exact in every option mode, skipped axioms remain zero, and no emitting delta record is retained. Annotated, anonymous-scope-bearing, multi-root, nested, and other segmented forms remain unsupported |
| Kernel v71 bounded local-delta addendum | One unannotated canonical binary or ternary `EquivalentClasses` root is now admitted through the silent transaction over an `ALL` or one exact base `EXCLUDE` table only when the complete direct equivalence classifier returns ignored. The graph must reach no anonymous individual; normal and taxonomy-only modes preserve the constructor and ignored-shape counts, while asserted-taxonomy preserves the constructor count and suppresses the ignored count. Skipped axioms remain zero and no emitting delta record is retained. Projecting pair/aggregate, annotated, oversized, anonymous-scope-bearing, multi-root, nested, and other segmented forms remain unsupported |
...
3. expand the proven bounded segmented families to nested aliases/deltas and general no-copy
   traversal, then execute general exporters after the ABI-floor decision;
