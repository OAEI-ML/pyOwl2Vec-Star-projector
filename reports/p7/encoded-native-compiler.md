...
Date: 2026-07-21. Projector implementation through `d0c9036`. pyOWLCore candidate revision:
...
count participates in the exact raw-edge proof. Validated class `AnnotationAssertion` roots remain
silent when `include_literals=False`; when
enabled, every annotation root must have produced exactly one native edge or the whole call falls
back. Selected anonymous annotation values and anonymous assertion operands use the kernel's exact
axiom-derived blank-ID order, including anonymous axiom metadata; ignored anonymous class
assertions remain on the scalar fallback path. The native non-string rendering count produces the
...
- one hidden isolated iterator adapter that admits only the named taxonomy, supported direct
  ...
  policy, admits anonymous positive-assertion operands and fully selected anonymous annotation
  values/metadata, ignored property chains, silent ontology annotations and SWRL rules, and all 27
  exactly counted scalar-skipped axiom constructors; synthesizes exact `only_taxonomy`
  ignored-restriction, non-string-rendering, and grouped skipped-axiom diagnostics; and publishes
  exact batch/copy/materialization provenance after complete
  consumption. Public
...
| Focused native/dispatch/streaming/API/integration/contract tests | 316 passed |
| Complete projector test suite | 1,081 passed |
...
`only_taxonomy`, and `include_literals` combinations, conservative fallback for ignored
equivalence and ignored anonymous class assertions with declined-session cleanup,
...
silent validation versus fallback for unselected annotation properties, exact anonymous positive
assertion and selected-annotation value IDs including metadata-only blank-ID participation across
encounter/canonical/include/suppress options, retained fallback for ignored anonymous class
assertions, exact grouped skip diagnostics across all 27 supported scalar-skipped constructors,
silent ontology-annotation and SWRL admission, and resource failure without a partial report.
