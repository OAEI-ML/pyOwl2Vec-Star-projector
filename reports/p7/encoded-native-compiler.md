...
| Direct/mmap/overlay/composite support | Exact full bytes and the canonical eleven-column packed direct-bytes arena are supported. Bounded empty-overlay aliases, one-root local deltas, dynamic exact-direct composites with source-local selectors, and reviewed nested-overlay composite families execute without flattening; the detailed sections above define their narrower annotation and scope-map envelopes. Kernel v121 applies independent source-local `EXCLUDE` layers to one nested overlay base and either or both of its two distinct direct siblings. Kernel v122 additionally applies an outer nested-member `EXCLUDE` to that overlay's exact local delta table in the one-direct-sibling family. Kernel v119 isolates valid general exporters at one future `PyUntypedBuffer` retention seam and proves mmap cleanup plus hostile-layout rejection without copying, but the pinned `abi3-py310` floor cannot use the Stable-ABI buffer lease introduced in 3.11. General recursive segmented traversal, nested-member selection outside the v122 one-direct local-delta family, nested aliases/deltas beyond the bounded families, a third direct sibling in this nested family, mmap execution, annotation-sensitive aliases outside the reviewed plans, and public dispatch remain unsupported by the Rust path |
