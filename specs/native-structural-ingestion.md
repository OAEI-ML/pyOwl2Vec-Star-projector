...
them as role axioms. The unary named-or-inverse `FunctionalObjectProperty` and
`InverseFunctionalObjectProperty` constructors, plus `ReflexiveObjectProperty` and
`IrreflexiveObjectProperty`, follow the same non-projecting path.
