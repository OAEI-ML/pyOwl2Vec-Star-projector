...
Kernels v48–v60 add one bounded two-segment `OVERLAY_BASE`/`OVERLAY_DELTA` slice. The base may use
...
`DisjointDataProperties` set, or a named-property `DataPropertyDomain` over the existing
recursive class-expression envelope. The canonical merger derives one insertion scalar without
