...
Kernels v48–v61 add one bounded two-segment `OVERLAY_BASE`/`OVERLAY_DELTA` slice. The base may use
...
class-expression envelope, or a named-property `DataPropertyRange` over the existing recursive
data-range envelope. The canonical merger derives one insertion scalar without flattening or
