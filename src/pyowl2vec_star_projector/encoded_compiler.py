...
class _EncodedColumns:
    ...
    def inspect_root(self, root_index: int, inspection: _Inspection) -> None:
        ...
        elif tag == _TAG_SYMMETRIC_OBJECT_PROPERTY:
        ...
        elif tag == _TAG_DATA_PROPERTY_ASSERTION:
            counters.data_property_assertion_axioms += 1
        elif tag == _TAG_NEGATIVE_DATA_PROPERTY_ASSERTION:
        ...
        elif tag == _TAG_ANNOTATION_PROPERTY_RANGE:
            counters.annotation_property_range_axioms += 1
        else:
    ...
    def _inspect_node(self, node_id: int, inspection: _Inspection) -> None:
        ...
        if tag in {_TAG_EQUIVALENT_DATA_PROPERTIES, _TAG_DISJOINT_DATA_PROPERTIES}:
            ...
            self._annotation_set_range(start + 1)
            return
        if tag == _TAG_DATA_PROPERTY_DOMAIN:
            start = self._exact_fields(node_id, 3)
            if not self._is_named_data_property(self._field_node(start)):
                raise SnapshotCompatibilityError(
                    "encoded subset DataPropertyDomain property is not a data property"
                )
            if not self._is_validated_class_expression(self._field_node(start + 1)):
                inspection.fallback(
                    "encoded subset requires a validated class expression in DataPropertyDomain"
                )
            self._annotation_set_range(start + 2)
            return
        ...
        if tag == _TAG_NEGATIVE_OBJECT_PROPERTY_ASSERTION:
            ...
            self._annotation_set_range(start + 3)
            return
        if tag == _TAG_DATA_PROPERTY_ASSERTION:
            start = self._exact_fields(node_id, 4)
            if not self._is_named_data_property(self._field_node(start)):
                raise SnapshotCompatibilityError(
                    "encoded subset DataPropertyAssertion property is not a data property"
                )
            if not self._is_supported_individual(self._field_node(start + 1)):
                raise SnapshotCompatibilityError(
                    "encoded subset DataPropertyAssertion source is not an individual"
                )
            if self.node_tag(self._field_node(start + 2)) != _TAG_LITERAL:
                raise SnapshotCompatibilityError(
                    "encoded subset DataPropertyAssertion value is not a literal"
                )
            self._annotation_set_range(start + 3)
            return
        if tag == _TAG_NEGATIVE_DATA_PROPERTY_ASSERTION:
            start = self._exact_fields(node_id, 4)
            if not self._is_named_data_property(self._field_node(start)):
                raise SnapshotCompatibilityError(
                    "encoded subset NegativeDataPropertyAssertion property is not a data property"
                )
            if not self._is_supported_individual(self._field_node(start + 1)):
                raise SnapshotCompatibilityError(
                    "encoded subset NegativeDataPropertyAssertion source is not an individual"
                )
            if self.node_tag(self._field_node(start + 2)) != _TAG_LITERAL:
                raise SnapshotCompatibilityError(
                    "encoded subset NegativeDataPropertyAssertion value is not a literal"
                )
            self._annotation_set_range(start + 3)
            return
        ...
        if tag == _TAG_ANNOTATION_PROPERTY_DOMAIN:
            ...
            self._annotation_set_range(start + 2)
            return
        if tag == _TAG_ANNOTATION_PROPERTY_RANGE:
            start = self._exact_fields(node_id, 3)
            if not self._is_named_annotation_property(self._field_node(start)):
                raise SnapshotCompatibilityError(
                    "encoded subset AnnotationPropertyRange property is not an annotation property"
                )
            if self.node_tag(self._field_node(start + 1)) != _TAG_IRI:
                raise SnapshotCompatibilityError(
                    "encoded subset AnnotationPropertyRange range is not an IRI"
                )
            self._annotation_set_range(start + 2)
            return
        inspection.fallback(
...
def _skipped_axiom_index(roots: tuple[_EncodedRootRef, ...]) -> dict[str, int]:
    constructors = {
        ...
        _TAG_ANNOTATION_PROPERTY_RANGE: "AnnotationPropertyRange",
    }
    result: dict[str, int] = {}
    for root in roots:
