...
def test_functional_object_property_corruption_fails_before_output(corruption: str) -> None:
    view = _snapshot(
        'FunctionalObjectProperty(Annotation(<urn:a> "x") ObjectInverseOf(:p)) '
        "SubObjectPropertyOf(ObjectPropertyChain(:p :q) :s) "
        "SubClassOf(:A :B) Declaration(Class(:Wrong))"
    )
    lease = _lease(view)
    columns = _EncodedColumns(lease)
    axiom_id = next(
        node_id for node_id in range(1, columns.node_count + 1) if columns.node_tag(node_id) == 76
    )
    chain_id = next(
        node_id for node_id in range(1, columns.node_count + 1) if columns.node_tag(node_id) == 11
    )
    ...
    with pytest.raises(
        ...
        match=(
            ...
            r"FunctionalObjectProperty property"
        ),
    ):
        prepare_encoded_subset_compilation(
            view,
