...
def test_data_property_assertion_corruption_fails_before_output(corruption: str) -> None:
    view = _snapshot(
        'DataPropertyAssertion(Annotation(<urn:a> "meta") :dp _:source '
        '"7"^^<http://www.w3.org/2001/XMLSchema#integer>) '
        "SubClassOf(:A :B) Declaration(Class(:Wrong))"
    )
    lease = _lease(view)
    columns = _EncodedColumns(lease)
    axiom_id = next(
        node_id for node_id in range(1, columns.node_count + 1) if columns.node_tag(node_id) == 115
    )
    wrong_id = next(
        node_id
        for node_id in range(1, columns.node_count + 1)
