...
def test_asserted_taxonomy_skips_other_supported_axiom_edges() -> None:
    view = _snapshot(
        ...
        "DisjointUnion(:definedUnion :unionA ObjectSomeValuesFrom(:unionP :unionB)) "
...
def test_disjoint_unions_match_scalar_skips_and_blank_ids() -> None:
    view = _snapshot(
        "DisjointUnion(Annotation(<urn:meta> _:skippedMeta) :DefinedA :A "
        "ObjectSomeValuesFrom(:p :B) ObjectUnionOf(:C :D)) "
        "DisjointUnion(:DefinedB :E :F) DisjointClasses(:PriorA :PriorB) "
        "SubObjectPropertyOf(:child :p) ObjectPropertyDomain(:p :Domain) "
        "ObjectPropertyRange(:p :Range) ObjectPropertyAssertion(:u _:edge :i)"
    )
    lease = _lease(view)
    ...
    with (
        ...
        patch.object(
            api_module,
            "prepare_streaming_compilation",
            side_effect=AssertionError("DisjointUnion slice crossed scalar traversal"),
        ),
    ):
        for options, (scalar_edges, scalar_report) in zip(cases, expected, strict=True):
            projector = Projector()
            ...
            assert counters is not None
            assert counters.roots_inspected == 7
            assert counters.disjoint_union_axioms == 2
            assert counters.disjoint_class_axioms == 1
            assert counters.sub_object_property_axioms == 1
            assert counters.object_property_assertion_axioms == 1
            assert counters.anonymous_individuals == 2
...
def test_skipped_disjoint_union_does_not_mutate_scala_instance_role_state() -> None:
    skipped_view = _snapshot(
        'DisjointUnion(Annotation(<urn:meta> "skipped") :Defined :shared '
        "ObjectSomeValuesFrom(:shared :C))"
    )
    domain_range_view = _snapshot(
        "ObjectPropertyDomain(:shared :D) ObjectPropertyRange(:shared :R)"
    )
    ...
    with (
        ...
        patch.object(
            api_module,
            "prepare_streaming_compilation",
            side_effect=AssertionError("encoded DisjointUnion lifecycle crossed scalar"),
        ),
    ):
        assert projector.project(skipped_view, options=replace(options, backend="native")) == []
    ...
    first_counters = projector.last_encoded_counters
    assert first_counters is not None
    assert first_counters.disjoint_union_axioms == 1
    assert first_counters.scalar_fallbacks == 0
    ...
    with (
        ...
        patch.object(
            api_module,
            "prepare_streaming_compilation",
            side_effect=AssertionError("encoded DisjointUnion follow-on crossed scalar"),
        ),
    ):
        actual = projector.project(
            domain_range_view,
...
def test_segmented_disjoint_unions_preserve_skips_edges_and_leases() -> None:
    source_body = (
        "DisjointUnion(Annotation(<urn:meta> _:sourceMeta) :SourceDefined :A "
        "ObjectSomeValuesFrom(:q :B)) ObjectPropertyDomain(:p :D) "
        "ObjectPropertyAssertion(:u :i :j)"
    )
    delta_body = (
        "DisjointUnion(Annotation(<urn:meta> _:deltaMeta) :DeltaDefined :C "
        "ObjectUnionOf(:E :F)) ObjectPropertyRange(:p :R) SubObjectPropertyOf(:child :p)"
    )
    source = _snapshot(source_body)
    delta = _snapshot(delta_body)
    overlay = _snapshot(f"{source_body} {delta_body}")
    ...
    for view, lease, retained_owner_ids in rows:
        ...
        with (
            ...
            patch.object(
                api_module,
                "prepare_streaming_compilation",
                side_effect=AssertionError("segmented DisjointUnion crossed scalar traversal"),
            ),
        ):
            projector = Projector()
            actual = projector.project(view, options=replace(options, backend="native"))
...
def test_disjoint_union_corruption_fails_before_output(corruption: str) -> None:
    view = _snapshot(
        'DisjointUnion(Annotation(<urn:a> "x") :Defined :A '
        "ObjectSomeValuesFrom(:p :B)) SubClassOf(:Projected :Edge) "
        "Declaration(NamedIndividual(:Wrong))"
    )
    lease = _lease(view)
    columns = _EncodedColumns(lease)
    axiom_id = next(
        node_id for node_id in range(1, columns.node_count + 1) if columns.node_tag(node_id) == 64
    )
    wrong_id = next(
        node_id
        for node_id in range(1, columns.node_count + 1)
    ...
    set_index = field_index + 1
    set_item = int.from_bytes(
        buffers["field_values"][set_index * 8 : (set_index + 1) * 8],
        "little",
    )
    annotation_index = field_index + 2
    annotation_item = int.from_bytes(
    ...
    if corruption == "arity":
        ...
        changed_offsets[end_offset : end_offset + 8] = (end - 1).to_bytes(8, "little")
        buffers["node_field_offsets"] = memoryview(bytes(changed_offsets))
    elif corruption in {"defined-kind", "set-kind", "annotation-kind"}:
        kinds = bytearray(buffers["field_kinds"])
        index = {
            "defined-kind": field_index,
            "set-kind": set_index,
            "annotation-kind": annotation_index,
        }[corruption]
        kinds[index] = 7 if corruption == "defined-kind" else 1
        buffers["field_kinds"] = memoryview(bytes(kinds))
    elif corruption == "defined-length":
        lengths = bytearray(buffers["field_lengths"])
        lengths[field_index * 8 : (field_index + 1) * 8] = (1).to_bytes(8, "little")
        buffers["field_lengths"] = memoryview(bytes(lengths))
    elif corruption in {"defined-reference", "wrong-defined"}:
        values = bytearray(buffers["field_values"])
        replacement = columns.node_count + 1 if corruption == "defined-reference" else wrong_id
        values[field_index * 8 : (field_index + 1) * 8] = replacement.to_bytes(8, "little")
        buffers["field_values"] = memoryview(bytes(values))
    else:
        values = bytearray(buffers["item_values"])
    ...
    with pytest.raises(
        SnapshotCompatibilityError,
        match=(
            r"arity|node reference|canonical set|DisjointUnion (?:defined class|item)|"
            r"annotation set"
        ),
    ):
        prepare_encoded_subset_compilation(
